...
class TestForecastingService:
	...
	def test_sales_forecast_enough_data(self, mock_get):
		...
		analytics = [make_analytics(date=base_date + timedelta(days=i), sales=100+i, orders=10+i) for i in range(30)]
	...
	def test_sales_forecast_not_enough_data(self, mock_get):
		...
		analytics = [make_analytics(date=base_date + timedelta(days=i), sales=100, orders=10) for i in range(3)]
		mock_get.return_value = analytics
		with self.client.application.test_request_context():
			resp = forecasting_service.get_sales_forecast(shop_id=self.shop.shop_id)
		assert resp.status_code == 200
	...
	def test_sales_forecast_with_segment(self, mock_get):
		base_date = datetime(2025, 12, 1)
		analytics = [make_analytics(date=base_date + timedelta(days=i), sales=100+i, orders=10+i) for i in range(10)]
		mock_get.return_value = analytics
		with self.client.application.test_request_context():
